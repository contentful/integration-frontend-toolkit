...
const EntityForm = <T extends Entity>(props: Props<T>) => {
  ...
  const onSearch = (searchQuery: SearchParams) => {
    const newList = searchQuery.searchValue
      ? searchableList.filter((entity: Entity) => {
  ...
  const listContent = showSelected
    ? isMultiSelect()
      ? selectedMultiple
      : [selected]
    : searchableList;
      ...
      <Form onSubmit={() => onSubmit && onSubmit(isMultiSelect() ? selectedMultiple : selected)}>
        ...
        <Flex className={styles.controls} alignItems="center" justifyContent="space-between">
          <TextLink onClick={() => setShowSelected(!showSelected)}>
            {showSelected
              ? 'Show all'
              : `Show Selected ${isMultiSelect() ? '(' + selectedMultiple.length + ')' : ''}`}
          </TextLink>
          <TextLink onClick={clearSelected}>Clear Selected</TextLink>
        </Flex>
